...
public:
    ...
    HASH_INDEX_T next() 
    {
        ...
        HASH_INDEX_T next = ((this->start_)+(this->numProbes_ * this->dhstep_)) % this->m_;
        this->numProbes_++;
        return next;
       ...
       :  hash_(hash), kequal_(kequal), prober_(prober)
{
    ...
    this->newAlpha = resizeAlpha;
    this->filled = 0; //to account for places where things were deleted
    this->mIndex_ = 0;
    this->table_.assign(11, NULL); //failing tests without this? found on stackOverflow
...
void HashTable<K,V,Prober,Hash,KEqual>::insert(const ItemType& p)
{
    if(find(p.first)!=NULL){
        ...
        //check to see if it needs to be resized, if the loading factor is greater than the threshold
        if(this->filled/(double)this->table_.size() >= this->newAlpha){
            this->resize();
        }
        
        //if no location is available to insert the item (should never happen since probing)
        if(this->filled >= this->table_.size()){
            throw std::logic_error("No location available");
        }

        //hash the item
        HashItem* obj = new HashItem(p);
        this->table_[probe(p.first)]=obj;
        this->filled++;
...
{
    std::vector<HashItem*> old = this->table_;
    std::vector<HashItem*> newTable;

    //fill new with NULL spots
    for(unsigned int i=0; i<CAPACITIES[this->mIndex_+1]; i++){
    ...
    //have to go through all of them to re-hash
    for(unsigned int i=0; i<old.size(); i++){

        if(old[i]!=NULL && old[i]->deleted == true){ //if something was previously deleted, remove it for good now
            this->filled = this->filled - 1;
            delete old[i];
        }
        else if(old[i]!=NULL && old[i]->deleted==false){
            HashItem* newItem = new HashItem(old[i]->item);
            this->table_[probe(old[i]->item.first)] = newItem;
        }
    }
